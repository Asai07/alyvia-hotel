...
const ServiceBookingPage = () => {
    ...
    return (
        <div className="bg-[#F9F9F7] min-h-screen font-sans selection:bg-[#2C342C] selection:text-white">
            ...
            {/* 1. CAMBIO: Contenedor más ancho (max-w-6xl) */}
            <div className="max-w-6xl mx-auto px-6 py-12">
                ...
                {/* 2. CAMBIO: Grid de 2 columnas iguales en pantallas grandes (lg:grid-cols-2) */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">

                    {/* COLUMNA IZQUIERDA: RESUMEN */}
                    <div>
                        <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-lg border border-[#2C342C]/5 sticky top-32">
                            {/* 3. CAMBIO: Imagen controlada (aspect-video en desktop) */}
                            ...
                            <h2 className="font-serif text-3xl text-[#2C342C] mb-4">{serviceData.title}</h2>
                            <p className="text-gray-500 mb-6 border-b border-gray-100 pb-6 leading-relaxed">
                        ...
                        <AnimatePresence mode="wait">
                                    ...
                                    <form onSubmit={handleAuth} className="space-y-6">
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" required className="w-full bg-[#F9F9F7] border border-gray-200 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[#C5A880]" placeholder="Ej: Smith" value={guestInfo.lastName} onChange={e => setGuestInfo({ ...guestInfo, lastName: e.target.value })} />
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="text" required className="w-full bg-[#F9F9F7] border border-gray-200 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[#C5A880]" placeholder="Ej: 101" value={guestInfo.roomNumber} onChange={e => setGuestInfo({ ...guestInfo, roomNumber: e.target.value })} />
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-4 flex flex-col gap-4">
                                            <button type="submit" disabled={loading} className="w-full bg-[#2C342C] text-white py-4 rounded-xl font-bold uppercase text-xs tracking-[0.2em] hover:bg-[#1A211B] transition-all disabled:opacity-70 flex justify-center">{loading ? "Verificando..." : "Verificar Estancia"}</button>
                                            <button type="button" onClick={() => navigate('/booking')} className="text-xs text-gray-400 hover:text-[#C5A880] underline text-center">¿Aún no eres huésped? Reserva una suite aquí.</button>
                                        </div>
                                    </form>
                                </motion.div>
                            )}
                            ...
                            {step === 2 && (
                                ...
                                >
                                    ...
                                    <p className="text-gray-500 mb-8 text-sm">Hola, <strong>Familia {guestInfo.lastName}</strong>. Estás a un paso de confirmar.</p>
                                    ...
                                    <form onSubmit={handleBooking} className="space-y-6">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input type="date" required className="w-full bg-[#F9F9F7] border border-gray-200 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[#C5A880] text-sm text-gray-600" onChange={e => setBookingDetails({ ...bookingDetails, date: e.target.value })} />
                                                </div>
                                            </div>
                                            <div>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <select required className="w-full bg-[#F9F9F7] border border-gray-200 rounded-xl py-3 pl-12 pr-4 focus:outline-none focus:border-[#C5A880] text-sm text-gray-600 appearance-none" onChange={e => setBookingDetails({ ...bookingDetails, time: e.target.value })}>
                                        ...
                                        <div>
                                            <label className="text-xs font-bold uppercase tracking-widest text-gray-400 block mb-2">Notas Especiales</label>
                                            <textarea rows="3" className="w-full bg-[#F9F9F7] border border-gray-200 rounded-xl p-4 focus:outline-none focus:border-[#C5A880] text-sm text-gray-600 resize-none" placeholder="Opcional..." onChange={e => setBookingDetails({ ...bookingDetails, notes: e.target.value })}></textarea>
                                        </div>
                                        ...
                                        <button type="submit" disabled={loading} className="w-full bg-[#2C342C] text-white py-4 rounded-xl font-bold uppercase text-xs tracking-[0.2em] hover:bg-[#1A211B] transition-all flex justify-center mt-4 shadow-lg">{loading ? "Procesando..." : "Confirmar Reserva"}</button>
                                    </form>
                                </motion.div>
                            )}
                            ...
                            {step === 3 && (
                                ...
                                >
                                    ...
                                    <p className="text-gray-500 max-w-md mb-8 leading-relaxed">Tu experiencia <strong>{serviceData.title}</strong> ha sido agendada.</p>
